...
export default function CreatePage() {
  ...
  const openWithPaste = useCallback((pastedText: string) => {
    ...
    if (looksLikeUrl(text)) { setLink(text); setCategory('links'); }
    setOpen(true);
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      const body: Record<string, string> = { title: title.trim(), link: link.trim(), category };
  ...
  const joinRoom = async () => {
    ...
    const d = await res.json();
    if (res.ok) {
      setRooms(prev => prev.find(r => r.id === d.room.id) ? prev : [...prev, d.room]);
      setSelectedRoomId(d.room.id);
      setJoinCode('');
  ...
  const deleteRoom = async () => {
    ...
    if (res.ok) {
      ...
      if (selectedRoomId === deleteTarget.id) setSelectedRoomId('private');
      setDeleteTarget(null);
      setDeleteConfirmName('');
      setRoomMsg('Room deleted.');
    } else {
      const d = await res.json();
      setRoomMsg(d.error || 'Failed to delete room');
    }
  };

  const selectedRoom = rooms.find(r => r.id === selectedRoomId);

  const inputClass = "bg-transparent text-foreground border-secondary border-t border-l border-r-6 border-b-6 px-3 py-2 text-sm h-10";
  const btnClass = "px-3 py-2 text-sm text-foreground border-secondary border-t border-l border-r-6 border-b-6 h-10 inline-flex items-center gap-1 shrink-0";
      ...
      <div className="flex items-center justify-center h-screen w-screen">
        <div className="text-center w-full max-w-sm sm:max-w-md md:max-w-xl lg:max-w-2xl xl:max-w-3xl 2xl:max-w-4xl mx-4 md:mx-auto p-12 md:p-24 border-secondary border-t border-l border-r-6 border-b-6">
          <div className="md:hidden space-y-10">
            <Button variant="outline" size="lg" onClick={openMenu} className="w-full h-28 px-16 text-2xl text-foreground border-foreground border-t border-l border-r-6 border-b-6">UPLOAD</Button>
            <div className="text-foreground/60">OR</div>
            <Button variant="outline" size="lg" onClick={openMenu} className="w-full h-28 px-16 text-2xl text-foreground border-secondary border-t border-l border-r-6 border-b-6">PASTE</Button>
          </div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-3xl md:max-w-5xl p-8 border-secondary border-t border-l border-r-6 border-b-6 max-h-[90vh] overflow-y-auto">
          ...
          <div className="mb-8 flex items-center gap-4 flex-wrap">
            ...
            <button type="button" onClick={() => setShowRoomPanel(!showRoomPanel)}
              className="flex items-center gap-1 text-accent text-sm hover:underline">
              <Users className="w-4 h-4" />{showRoomPanel ? 'Hide rooms' : 'Manage rooms'}
            </button>
            ...
            <div className="mb-8 p-4 border-secondary border-t border-l border-r-6 border-b-6 space-y-4">
              ...
              <div className="flex gap-2 items-stretch">
                <input placeholder="Room name" value={newRoomName} onChange={(e) => setNewRoomName(e.target.value)}
                  onKeyDown={(e) => e.key === 'Enter' && createRoom()} className={`flex-1 ${inputClass}`} />
                <button type="button" onClick={createRoom} className={btnClass}><Plus className="w-4 h-4" /> Create</button>
              </div>

              <div className="flex gap-2 items-stretch">
                <input placeholder="Invite code" value={joinCode} onChange={(e) => setJoinCode(e.target.value.toUpperCase())}
                  onKeyDown={(e) => e.key === 'Enter' && joinRoom()} maxLength={8}
                  className={`flex-1 ${inputClass} uppercase`} />
                <button type="button" onClick={joinRoom} className={btnClass}>Join</button>
              </div>
                    ...
                    <div key={r.id} className="flex items-center justify-between text-sm gap-2">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <button type="button" onClick={() => copyCode(r.code)}
                          className="text-foreground/40 text-xs hover:text-accent transition-colors flex items-center gap-1 cursor-pointer">
                          {copiedCode === r.code ? <Check className="w-3 h-3 text-accent" /> : <Copy className="w-3 h-3" />}
                          {r.code}
                        </button>
                        {r.ownerId && (
                          <button type="button" onClick={() => { setDeleteTarget(r); setDeleteConfirmName(''); }}
                            className="text-foreground/20 hover:text-destructive transition-colors" title="Delete room">
              ...
              {deleteTarget && (
                <div className="p-3 border-destructive/30 border-t border-l border-r-6 border-b-6 space-y-2">
                  <div className="text-destructive text-sm">
                    Delete <strong>{deleteTarget.name}</strong>? All shared items will be lost. Type the room name to confirm.
                  </div>
                  <div className="flex gap-2 items-stretch">
                    <input value={deleteConfirmName} onChange={(e) => setDeleteConfirmName(e.target.value)}
                      placeholder={deleteTarget.name} className={`flex-1 ${inputClass}`} />
                    <button type="button" onClick={deleteRoom}
                      disabled={deleteConfirmName !== deleteTarget.name}
                      className={`${btnClass} text-destructive border-destructive/50 disabled:opacity-30`}>
                      Delete
                    </button>
                    <button type="button" onClick={() => { setDeleteTarget(null); setDeleteConfirmName(''); }}
                      className={btnClass}>Cancel</button>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="mt-10 flex justify-end gap-6">
            <Button variant="outline" className="px-8 py-3 text-xl text-foreground border-foreground border-t border-l border-r-6 border-b-6"
              onClick={() => { setOpen(false); setError(''); }}>CANCEL</Button>
            <Button variant="outline" className="px-8 py-3 text-xl text-foreground border-secondary border-t border-l border-r-6 border-b-6 disabled:opacity-50"
